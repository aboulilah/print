...
const CATEGORIES = [
  {
    ...
    products: [
      ...
      { name: "Transparent Sticker", desc: "Clear film, great for glass & packaging.", icon: ICONS.sticker },
      { name: "Vinyl Sticker", desc: "Durable, weatherproof, cut to any shape.", icon: ICONS.sticker },
      { name: "White PP Sticker", desc: "Opaque white base, sharp colour print.", icon: ICONS.sticker },
      { name: "Silver Sticker", desc: "Metallic silver finish for a bold look.", icon: ICONS.sticker }
    ]
  },
  {
    id: "banner",
    ...
    products: [
      ...
      { name: "Bunting Flags", desc: "String buntings for launches & events.", icon: ICONS.banner },
      { name: "Roll-Up Banner", desc: "Portable stand banner for booths & events.", icon: ICONS.banner },
      { name: "Event Backdrop", desc: "Large-format backdrop for stage & photo walls.", icon: ICONS.banner }
    ]
  },
  {
    id: "booklet",
    ...
    products: [
      ...
      { name: "Company Profile Book", desc: "Multi-page saddle-stitch or perfect bound.", icon: ICONS.booklet },
      { name: "Product Catalog", desc: "Full-colour catalog for your product range.", icon: ICONS.booklet },
      { name: "Guide / Manual Book", desc: "Manuals, guides, workbook printing.", icon: ICONS.booklet }
    ]
  },
  {
    id: "packaging",
    ...
    products: [
      ...
      { name: "Paper Bag", desc: "Branded carrier bags, various sizes.", icon: ICONS.box },
      { name: "Food / Product Pouch", desc: "Stand-up pouches for FnB & retail.", icon: ICONS.box },
      { name: "Courier Bag", desc: "Custom-printed poly mailers.", icon: ICONS.box }
    ]
  },
  {
    id: "design",
    ...
    products: [
      ...
      { name: "Brand Identity Package", desc: "Logo, colours, fonts & guidelines.", icon: ICONS.design },
      { name: "Packaging Design", desc: "Print-ready artwork for box or label.", icon: ICONS.design },
      { name: "Social Media Design Set", desc: "Templates for posts, stories & ads.", icon: ICONS.design }
    ]
  },
  {
    id: "marketing",
    ...
    products: [
      ...
      { name: "Brochures", desc: "Tri-fold or bi-fold, full colour.", icon: ICONS.flyer },
      { name: "Table Calendar", desc: "Corporate desk calendars, custom design.", icon: ICONS.flyer },
      { name: "Posters", desc: "A3 to large-format promotional posters.", icon: ICONS.flyer }
    ]
  },
  {
    id: "namecard",
    ...
    products: [
      ...
      { name: "Premium Textured Card", desc: "Cotton / linen stock, embossed feel.", icon: ICONS.namecard },
      { name: "Metal Card", desc: "Stainless steel cards for a bold impression.", icon: ICONS.namecard },
      { name: "QR Digital Name Card", desc: "Card with scannable digital profile.", icon: ICONS.namecard }
    ]
  },
  {
    id: "gifts",
    ...
    products: [
      ...
      { name: "Corporate Tumbler", desc: "Branded tumblers for staff & clients.", icon: ICONS.gift },
      { name: "Tote Bag", desc: "Canvas tote, screen or full-colour print.", icon: ICONS.gift },
      { name: "Lanyard", desc: "Custom woven or printed lanyards.", icon: ICONS.gift }
    ]
  }
];
...
function renderProducts() {
  ...
  state.category.products.forEach(p => {
    ...
    card.innerHTML = `
      <div class="prod-icon" style="color: var(--cyan);">${p.icon}</div>
      <div class="prod-body">
        <h4>${p.name}</h4>
        <p>${p.desc}</p>
